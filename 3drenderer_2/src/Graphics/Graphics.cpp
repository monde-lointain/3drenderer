...
void Graphics::clear_z_buffer()
{
	...
	// Set up a register with 8 zero float values
	__m256 one = _mm256_set1_ps(1.0f);

	// Eight pixels at a time until we have less than eight remaining
	for (i = 0; i < loop_count * 8; i += 8)
	{
		_mm256_store_ps(&depth_buffer[i], one);
	}
	...
	const int& y1, const uint32& color)
{
	...
	// x going up: 1, x going down: -1
	int x_inc = (x0 < x1) ? 1 : -1;
	// y going up: 1, y going down: -1
	int y_inc = (y0 < y1) ? 1 : -1;
	...
	while (true)
	{
		...
		// Stop once we've reached the end point of the line
...
void Graphics::draw_vertices(const Triangle& triangle, int point_size, const uint32& color)
{
	...
	for (glm::vec4 vertex : triangle.vertices)
	{
		...
		Graphics::draw_rect(
			x_pos, y_pos, point_size, point_size, color
		);
	}
}
